$(document).ready(function () {
    ...
    $("#addMalgruppeButton").click(function () {
        var itemCount = $("#tab4Div > div").length;
        var templateHtml = $("#tab4Div > div").last().html();
        var updatedHtml = templateHtml.replace(/\[\d+\]/g, "[" + itemCount + "]");

        $("#tab4Div").append("<div class='well'> " + updatedHtml + "</div>");
    });

    $("#removeMalgruppeButton").click(function () {
        var itemCount = $("#tab4Div > div").length;

        if (itemCount > 1) {
            $("#tab4Div > div").last().remove();
        }
    });
